...
reboot_os() {
    ...
    read -p "Apakah Kamu Ingin Merestart System? [y/n]" is_reboot
    if [[ ${is_reboot} == "y" || ${is_reboot} == "Y" ]]; then
        reboot
    else
        echo -e "${red}Info:${plain} Reboot Dibatalkan.."
        exit 0
    fi
}
...
install_bbr() {
    ...
    if [[ x"${release}" == x"centos" ]]; then
        ...
        echo -e "${red}Error:${plain} OS Tidak Didukung, Harap Berganti Ke CentOS/Debian/Ubuntu Dan Coba Kembali"
        exit 1
    fi
